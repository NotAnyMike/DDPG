...
with tf.variable_scope('main'):
    opt_action = create_nn(s_ph,hidden_sizes=hidden_sizes+[action_dim],scope='act', act_limit=max_act)
    opt_action_value = create_nn(
            tf.concat([a_ph,s_ph], axis=-1),
            scope='act-val', 
            reuse=None,
            hidden_sizes=hidden_sizes+[1])
    opt_action_value2 = create_nn(
            tf.concat([opt_action,s_ph], axis=-1),
            scope='act-val', 
            reuse=True,
            hidden_sizes=hidden_sizes+[1])
with tf.variable_scope('target'):
    opt_action_target = create_nn(s_ph, hidden_sizes+[action_dim], scope='act', act_limit=max_act)
    opt_action_value2_target = create_nn(
...
opt_act_value_train = opt_act_value_optimizer.minimize(loss_opt_act_val,var_list=get_vars('main/act-val'))
opt_action_train    = opt_action_optimizer.minimize(loss_opt_action,var_list=get_vars('main/act'))
...
# Count variables
var_counts = tuple(count_vars(scope) for scope in ['main/act', 'main/act-val', 'main'])
